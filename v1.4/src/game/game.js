...
export class Game {
  ...
  _camera(dt) {
    ...
    const t = THREE.MathUtils.clamp(surf ? surf.timeOfImpact - 0.1 : tMax, tMin, tMax);
  ...
  _target(snap) {
    ...
    if (this.heldId) lines.push('Click: put down where you aim', 'Hold F or right-click: throw');
    else {
      const cam = this.stage.camera.position;
      const dir = this._aim(tmp2);
      const chest = tmp.copy(this.pos).setY(this.pos.y + this.H * 0.6).clone();
      const reach = 2.9 * s + this.r;
      let best = null;
      let bestScore = Infinity;
      const consider = (obj, station) => {
        const c = obj.position;
        if (c.distanceTo(chest) > reach) return;
        ...
        const slack = station ? 0.2 : 0.17;
        if (ang > slack && off > (station ? 0.5 : 0.3)) return;
        const score = ang + along * 0.004;
        if (score < bestScore) { bestScore = score; best = { obj, station }; }
      };
      for (const v of this.items.values()) if (v.obj.visible) consider(v.obj, null);
      for (const pr of this.proxies) consider(pr, pr.userData.station);
      if (best) {
        if (best.station) {
