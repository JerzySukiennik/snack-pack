...
class LocalRoom extends Base {
  ...
  _recv(m) {
    ...
    else if (m.type === 'start') this.emit('start', m.data);
...
class FirebaseRoom extends Base {
  ...
  async open(create) {
    ...
    this.unsubs.push(db.onValue(ref(`${this.base}/meta`), (snap) => {
      const m = snap.val();
      if (m && m.state === 'started' && m.gameHost) this.emit('start', { host: m.gameHost, gen: m.gameGen });
    }));
    return this;
  }
  ...
  start(info) {
    this.fb.db.update(this.fb.ref(`${this.base}/meta`), { gameHost: info.host, gameGen: info.gen, state: 'started' }).catch((e) => console.warn('[room] start failed', e.message));
  }
