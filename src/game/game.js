...
export class Game {
  ...
  _event(kind, d) {
    ...
    } else if (kind === 'to' && d.k === 'assemble') this._miniStart(d.cook);
    else if (kind === 'fx') {
      if (d.k === 'pay' || d.k === 'lost') {
  ...
  primary(down) {
    ...
    if (!down) return;
    if (this.heldId) { this._send({ a: 'drop' }); return; }
    const t = this.target;
    if (!t) return;
  ...
  _move(dt) {
    ...
    this.vy = this.ctrl.computedGrounded() ? -1 : this.vy - 22 * dt;
    move.y = this.vy * dt;
    ...
    this.me.root.position.copy(this.pos);
    const moving = x || z;
    if ((moving || this.heldId) && !this.mini) {
  ...
  _camera(dt) {
    ...
    if (this.mini) {
      const b = this.world.layout.stations.board.c;
      cam.position.lerp(tmp.set(b[0], BOARD_Y + 1.05, b[2] - 0.75), 1 - Math.exp(-dt * 7));
      tmp2.set(b[0], BOARD_Y, b[2]);
    ...
    const want = pivot.clone().addScaledVector(aim, -3.1 * s).addScaledVector(right, 0.75 * s);
    want.y = Math.max(want.y + 0.3 * s, 0.35);
    cam.position.lerp(want, 1 - Math.exp(-dt * 22));
    cam.quaternion.setFromEuler(new THREE.Euler(this.pitch, this.yaw, 0, 'YXZ'));
    this.holdPoint.copy(pivot).addScaledVector(aim, 1.15 * s + this.r).setY(Math.max(pivot.y - this.H * 0.25 + aim.y * 1.2 * s, 0.2));
  }
  ...
  _applySnap(dt) {
    ...
    for (const it of b.items) {
      ...
      if (id === this.heldId) {
        v.obj.position.lerp(this.holdPoint, 1 - Math.exp(-dt * 18));
    ...
    for (const [id, pb] of Object.entries(b.players)) {
      ...
      r.creature.reach = held ? held.obj.position : pb[5] ? tmp.set(2.3, BOARD_Y + 0.1, 0.85).clone() : null;
    }
  ...
  _target(snap) {
    ...
    if (this.heldId) lines.push('Click: drop', 'Hold F or right-click: throw');
    else {
      this.rayc.setFromCamera({ x: 0, y: 0 }, this.stage.camera);
      const hit = this.rayc.intersectObjects([this.itemLayer, ...this.proxies], true)[0];
      if (hit) {
        const d = hit.point.distanceTo(tmp.copy(this.pos).setY(this.pos.y + this.H * 0.6));
        if (d < 2.3 * s + this.r) {
          if (hit.object.userData.station) {
            this.target = { station: hit.object.userData.station };
            lines.push(`Click: take a ${hit.object.userData.station === 'buns' ? 'bun' : 'sausage'}`);
          } else if (hit.object.userData.itemRoot) {
            const root = hit.object.userData.itemRoot;
            this.target = { id: root.userData.id };
            outline.push(root);
            lines.push(`Click: grab ${LABEL[root.userData.type] || 'item'}`);
          }
        }
      }
    }
    if (snap) {
      ...
      const order = dWin < 2.2 + this.r && snap.cust.some((c) => c[4] === 1 && !c[6]);
      const build = dBoard < 2.0 + this.r && snap.g[4] && !this.heldId;
  ...
  _miniStart(cook) {
    ...
    g.position.set(b[0], BOARD_Y + 0.02, b[2]);
    ...
    this.group.add(g);
    this.mini = { stage: 'press', g, bun, saus, bottle, cur: new THREE.Vector2(0.18, -0.12), t: 0, path: [], tube: null, down: false, off: [0, 0], ang: 0, press: 0, cook, drop: 0 };
    this.me.reach = new THREE.Vector3(b[0], BOARD_Y + 0.15, b[2]);
  }
