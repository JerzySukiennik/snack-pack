...
**v2 — the truck.** Ready ×everyone rolls the crew out to a food truck: over-the-shoulder camera with a dot crosshair, physics items you grab, carry and throw, a grill that burns things, a mouse-driven hot dog assembly mini-game whose result shows on the actual hot dog, customers with orders and patience, tickets, tips and a five-minute day. The host simulates physics (Rapier) and streams 20 Hz snapshots over WebRTC; Firebase only carries the lobby and signaling.
...
- `src/game/` — host simulation (`sim.js`), game client (`game.js`), HUD, world visuals, shared physics setup
- `src/net/peer.js` — host/client links (WebRTC DataChannels or BroadcastChannel)
...
- `tools/build_world.py` — regenerates `assets/world.glb` + `assets/world.json` (truck, plaza, props, colliders)
...
Truck: click to lock the mouse, WASD + Shift, click grabs/drops, hold F or right-click to throw, E takes orders and starts the hot dog mini-game, Q talks.
