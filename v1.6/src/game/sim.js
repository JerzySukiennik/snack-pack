...
export class Sim {
  ...
  action(id, m) {
    ...
    if (m.a === 'grab' && !p.held) {
      ...
      if (item) {
        ...
        if (near([t.x, t.y, t.z], 3.8)) this.hold(p, item);
      }
      ...
      if (item) {
        ...
        item.body.setRotation({ x: 0, y: Math.sin(yaw / 2), z: 0, w: Math.cos(yaw / 2) }, true);
      }
    }
    ...
    else if (m.a === 'combine') this.combine(p, m.id);
    else if (m.a === 'assembled') this.endAssemble(p, m);
  }

  takeOrder(p) {
    if (Math.hypot(p.pos[0], p.pos[2] - 1.0) > 3.2 + p.r) return;
  ...
  boardItems() {
    ...
    for (const it of this.items.values()) {
      ...
      if (it.type === 'sausage' && !sausage) sausage = it;
    }
    return { bun, sausage };
  }
  ...
  beginAssemble(p) {
    ...
    if (bun && sausage) this.startJob(p, bun, sausage);
  }

  combine(p, targetId) {
    if (this.assembling.has(p.id)) return;
    const held = this.items.get(p.held);
    const other = this.items.get(targetId);
    if (!held || !other || other.holder || other.lock) return;
    const pair = [held, other];
    const bun = pair.find((i) => i.type === 'bun');
    const sausage = pair.find((i) => i.type === 'sausage');
    if (!bun || !sausage) return;
    const t = other.body.translation();
    if (Math.hypot(t.x - p.pos[0], t.z - p.pos[2]) > 3.8 + p.r) return;
    this.release(p);
    this.startJob(p, bun, sausage, [t.x, t.y, t.z]);
  }

  startJob(p, bun, sausage, at) {
    const t = bun.body.translation();
    const pos = at || [t.x, t.y, t.z];
    bun.lock = sausage.lock = p.id;
    bun.body.setEnabled(false);
    sausage.body.setEnabled(false);
    this.assembling.set(p.id, { bun, sausage, t: this.time, pos });
    this.emit('to', { id: p.id, k: 'assemble', cook: sausage.cook, pos });
  }

  endJob(job, restore) {
    for (const it of [job.bun, job.sausage]) {
      it.lock = null;
      if (!this.items.has(it.id)) continue;
      it.body.setEnabled(true);
      if (restore) {
        const dx = it === job.sausage ? 0.22 : 0;
        it.body.setTranslation({ x: job.pos[0] + dx, y: job.pos[1] + 0.06, z: job.pos[2] }, true);
  ...
  endAssemble(p, m) {
    ...
    if (m.cancel || !this.items.has(job.bun.id) || !this.items.has(job.sausage.id)) return this.endJob(job, true);
    this.endJob(job, false);
    const clamp01 = (v) => Math.min(1, Math.max(0, +v || 0));
    const cook = job.sausage.cook;
    const cookQ = cook >= COOK.burnt || cook < COOK.done ? 0.1 : 1 - Math.min(Math.abs(cook - COOK.perfect) / 5, 0.6);
    ...
    this.spawn('hotdog', [job.pos[0], job.pos[1] + 0.08, job.pos[2]], meta);
