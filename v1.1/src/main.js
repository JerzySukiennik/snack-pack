...
function syncPlayers() {
  ...
  if (all && !starting && sorted[0][0] === room.selfId) {
    setTimeout(() => {
      const still = [...room.players.values()].every((p) => p.ready);
      if (still && !starting) room.start();
    }, 900);
  }
}
...
function runStart() {
  ...
  const anim = () => {
    const t = Math.min((performance.now() - t0) / 2200, 1);
    const e = t * t * (3 - 2 * t);
  ...
  $('bannerTitle').textContent = 'Crew assembled!';
  $('bannerText').textContent = 'The food truck rolls out in the next update.';
  setTimeout(() => $('banner').classList.add('on'), 1400);
  setTimeout(() => {
    $('banner').classList.remove('on');
    const t1 = performance.now();
    const close = () => {
      const t = Math.min((performance.now() - t1) / 1400, 1);
      door.position.y = (1 - t * t * (3 - 2 * t)) * 4.1;
      glow.intensity = (1 - t) * 28;
      if (t < 1) return requestAnimationFrame(close);
      stage.scene.remove(glow);
      starting = false;
      editor.enabled = true;
      setReady(false);
      if (room.unstart) room.unstart();
      focusMe();
    };
    close();
  }, 6200);
...
function bindUI() {
  ...
  window.addEventListener('pagehide', () => { if (me) persist(); if (room) room.leave(); });
}
...
async function boot() {
  ...
  $('create').disabled = $('join').disabled = false;
  window.SNACK = { stage, lib, me, editor, get room() { return room; }, remotes };
